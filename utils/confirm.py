...
def confirm_voice(transcript,prompt: str, retries: int = 1, record_seconds: int = 4 ):
    """
    Speak `prompt` and listen for a yes/no response.
     
    Returns True if confirmed or transcript is yes, False if denied or unclear after retries or         transcript is no.
